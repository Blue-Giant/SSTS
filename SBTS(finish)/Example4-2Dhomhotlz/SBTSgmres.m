function [x,flag,relres,iter,resvec] = SBTSgmres(A,b,restart,tol,maxit,M1,M2,parameter,x,varargin) 
% here: parameter is alpha,M1 is W, M2 is T
if (nargin < 2)
    error(message('SBTSgmres:NumInputs'));
end
 
[m,n] = size(A);
if (m ~= n)
    error(message('SBTSgmres:SquareMatrix'));
end
if ~isequal(size(b),[m,1])
    error(message('SBTSgmres:VectorSize', m));
end
...
warned = 0;
if tol < eps
    warning(message('MATLAB:SBTSgmres:tooSmallTolerance'));
    warned = 1;
    tol = eps;
elseif tol >= 1
    warning(message('MATLAB:SBTSgmres:tooBigTolerance'));
    warned = 1;
    tol = 1-eps;
end
...
if restarted
    outer = maxit;
    if restart > n
        warning(message('SBTSgmres:tooManyInnerItsRestart',restart, n));
        restart = n;
    end
    inner = restart;
else
    outer = 1;
    if maxit > n
        warning(message('SBTSgmres:tooManyInnerItsMaxit',maxit, n));
        maxit = n;
    end
    inner = maxit;
end
...
if (n2b == 0)                    % if    rhs vector is all zeros
    ...
    if (nargout < 2)
        itermsg('SBTSgmres',tol,maxit,0,flag,iter,NaN);
    end
    return
end
...
if ((nargin >= 10) && ~isempty(x))
    if ~isequal(size(x),[n,1])
        error(message('SBTSgmres:XoSize', n));
    end
else
    x = zeros(n,1);
end
...
if (normr <= tolb)               % Initial guess is a good enough solution
    ...
    resvec = normr;
    if (nargout < 2)
        itermsg('SBTSgmres',tol,maxit,[0 0],flag,iter,relres);
    end
    return
end
...
if existM
    ...
    tr1 = M1\r(1:n_size1);
    tsr = (r(n_size1+1:end) - M2*tr1)/parameter;
    ts1 =  M1\tsr;
    tr2 = M1*tr1;
    ts2 = (2*parameter-1)*M1*ts1;
    rnew2 = (1/parameter)*(M1\ts2);
    rnew1 = M1\(tr2+M2*rnew2);
    r = [rnew1;rnew2];
    
    if ~x0iszero
        %minv_b = iterapp('mldivide',m1fun,m1type,m1fcnstr,b,varargin{:});
        br1 = M1\r(1:n_size1);
        bsr = (r(n_size1+1:end) - M2*br1)/parameter;
        bs1 =  M1\bsr;
        br2 = M1*br1;
        bs2 = (2*parameter-1)*M1*bs1;
        b2 = (1/parameter)*(M1\bs2);
        b1 = M1\(br2+M2*b2);
        minv_b = [b1;b2];
    else
        minv_b = r;
...
for outiter = 1 : outer
    ...
    for initer = 1 : inner
        ...
        if existM
            ...
            vr1 = M1\v(1:n_size1);
            vsr = (v(n_size1+1:end) - M2*vr1)/parameter;
            vs1 =  M1\vsr;
            vr2 = M1*vr1;
            vs2 = (2*parameter-1)*M1*vs1;
            vnew2 = (1/parameter)*(M1\vs2);
            vnew1 = M1\(vr2+M2*vnew2);
            v = [vnew1;vnew2];         
            if ~all(isfinite(v))
                flag = 2;
                break
        ...
        if (normr <= tolb || stag >= maxstagsteps || moresteps)
            ...
            if existM
                ...
                vrr1 = M1\r(1:n_size1);
                vrsr = (r(n_size1+1:end) - M2*vrr1)/parameter;
                vrs1 =  M1\vrsr;
                vrr2 = M1*vrr1;
                vrs2 = (2*parameter-1)*M1*vrs1;
                vr2 = (1/parameter)*(M1\vrs2);
                vr1 = M1\(vrr2+M2*vr2);
                minv_r = [vr1;vr2];
            ...
            if normr_act <= tolb
                ...
                if moresteps >= maxmsteps
                    if ~warned
                        warning(message('SBTSgmres:tooSmallTolerance'));
                    end
                    flag = 3;
                    iter = [outiter, initer];
                    break;
    ...
    if flag ~= 0
        ...
        if existM
            %minv_r = iterapp('mldivide',m1fun,m1type,m1fcnstr,r,varargin{:});
            vr1r1 = M1\r(1:n_size1);
            vr1sr = (r(n_size1+1:end) - M2*vr1r1)/parameter;
            vr1s1 =  M1\vr1sr;
            vr1r2 = M1*vr1r1;
            vr1s2 = (2*parameter-1)*M1*vr1s1;
            vr12 = (1/parameter)*(M1\vr1s2);
            vr11 = M1\(vr1r2+M2*vr12);
            minv_r = [vr11;vr12];
            if ~all(isfinite(minv_r))
                flag = 2;
                break
...
if nargout < 2
    if restarted
        itermsg(sprintf('SBTSgmres(%d)',restart),tol,maxit,[outiter initer],flag,iter,relres);
    else
        itermsg(sprintf('SBTSgmres'),tol,maxit,initer,flag,iter(2),relres);
    end
end