function [x,flag,relres,iter,resvec] = PGSORgmres(A,b,restart,tol,maxit,M1,M2,parameter,parameter2,x,varargin) 
% here: parameter is alpha, parameter2 is omega, M1 is W, M2 is T, A is 
%  %%%%        %%%%  %%%%    %%%%     %%%%%%             %%%%%%
...
%   %%%%        %%%%  %% %%     %%%%    %%%% 
if (nargin < 2)
    error(message('PGSORgmres:NumInputs'));
end
 
[m,n] = size(A);
if (m ~= n)
    error(message('PGSORgmres:SquareMatrix'));
end
if ~isequal(size(b),[m,1])
    error(message('PGSORgmres:VectorSize', m));
end
...
warned = 0;
if tol < eps
    warning(message('MATLAB:PGSORgmres:tooSmallTolerance'));
    warned = 1;
    tol = eps;
elseif tol >= 1
    warning(message('MATLAB:PGSORgmres:tooBigTolerance'));
    warned = 1;
    tol = 1-eps;
end
...
if restarted
    outer = maxit;
    if restart > n
        warning(message('PGSORgmres:tooManyInnerItsRestart',restart, n));
        restart = n;
    end
    inner = restart;
else
    outer = 1;
    if maxit > n
        warning(message('PGSORgmres:tooManyInnerItsMaxit',maxit, n));
        maxit = n;
    end
    inner = maxit;
end
...
if (n2b == 0)                    % if    rhs vector is all zeros
    ...
    if (nargout < 2)
        itermsg('PGSORgmres',tol,maxit,0,flag,iter,NaN);
    end
    return
end
...
if ((nargin >= 10) && ~isempty(x))
    if ~isequal(size(x),[n,1])
        error(message('PGSORgmres:XoSize', n));
    end
else
    x = zeros(n,1);
end
...
if (normr <= tolb)               % Initial guess is a good enough solution
    ...
    resvec = normr;
    if (nargout < 2)
        itermsg('PGSORgmres',tol,maxit,[0 0],flag,iter,relres);
    end
    return
end
...
if existM
    ...
    ts = (r(n_size1+1:end) - parameter*TT*rnew1);
    rnew2 = WW\ts;
    r = [rnew1;rnew2];
    ...
    if ~x0iszero
        ...
        b1 = WW\b(1:n_size1) ;
        bs = (b(n_size1+1:end)-parameter*TT*b1);
        b2 = WW\bs;
        minv_b = [b1;b2];
    else
        minv_b = r;
...
for outiter = 1 : outer
    ...
    for initer = 1 : inner
        ...
        if existM
           ...
           vs = (v(n_size1+1:end)- parameter*TT*vnew1);
           vnew2 = WW\vs;
           v = [vnew1;vnew2];  
            if ~all(isfinite(v))
                flag = 2;
        ...
        if (normr <= tolb || stag >= maxstagsteps || moresteps)
            ...
            if existM
                ...
                vr1 = WW\r(1:n_size1);
                vrs = (r(n_size1+1:end)-parameter*TT*vr1);
                vr2 = WW\vrs;
                minv_r = [vr1;vr2];
            ...
            if normr_act <= tolb
                ...
                if moresteps >= maxmsteps
                    if ~warned
                        warning(message('PGSORgmres:tooSmallTolerance'));
                    end
                    flag = 3;
                    iter = [outiter, initer];
                    break;
    ...
    if flag ~= 0
        ...
        if existM
            %minv_r = iterapp('mldivide',m1fun,m1type,m1fcnstr,r,varargin{:});
            vr11 = WW\r(1:n_size1);
            vr2s = (r(n_size1+1:end)-parameter*TT*vr11);
            vr22 = WW\vr2s;
            minv_r = [vr11;vr22];
            if ~all(isfinite(minv_r))
                flag = 2;
...
if nargout < 2
    if restarted
        itermsg(sprintf('PGSORgmres(%d)',restart),tol,maxit,[outiter initer],flag,iter,relres);
    else
        itermsg(sprintf('PGSORgmres'),tol,maxit,initer,flag,iter(2),relres);
    end
end