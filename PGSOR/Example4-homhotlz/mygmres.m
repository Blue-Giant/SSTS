function [x,flag,relres,iter,resvec] = mygmres(A,b,restart,tol,maxit,M1,M2,x,varargin) 
if (nargin < 2)
    error(message('MATLAB:mygmres:NumInputs'));
end
 
[m,n] = size(A);
if (m ~= n)
    error(message('MATLAB:mygmres:SquareMatrix'));
end
if ~isequal(size(b),[m,1])
    error(message('MATLAB:mygmres:VectorSize', m));
end
...
warned = 0;
if tol < eps
    warning(message('MATLAB:mygmres:tooSmallTolerance'));
    warned = 1;
    tol = eps;
elseif tol >= 1
    warning(message('MATLAB:mygmres:tooBigTolerance'));
    warned = 1;
    tol = 1-eps;
end
...
if restarted
    outer = maxit;
    if restart > n
        warning(message('MATLAB:mygmres:tooManyInnerItsRestart',restart, n));
        restart = n;
    end
    inner = restart;
else
    outer = 1;
    if maxit > n
        warning(message('MATLAB:mygmres:tooManyInnerItsMaxit',maxit, n));
        maxit = n;
    end
    inner = maxit;
end
...
if (n2b == 0)                    % if    rhs vector is all zeros
    ...
    if (nargout < 2)
        itermsg('mygmres',tol,maxit,0,flag,iter,NaN);
    end
    return
end
...
if ((nargin >= 8) && ~isempty(x))
    if ~isequal(size(x),[n,1])
        error(message('MATLAB:mygmres:XoSize', n));
    end
else
    x = zeros(n,1);
end
...
if (normr <= tolb)               % Initial guess is a good enough solution
    ...
    resvec = normr;
    if (nargout < 2)
        itermsg('mygmres',tol,maxit,[0 0],flag,iter,relres);
    end
    return
end
minv_b = b;
...
if nargout < 2
    if restarted
        itermsg(sprintf('mygmres(%d)',restart),tol,maxit,[outiter initer],flag,iter,relres);
    else
        itermsg(sprintf('mygmres'),tol,maxit,initer,flag,iter(2),relres);
    end
end